...
if (!preg_match('/Ad{0,11}+z/u', $_POST['id'])) {
    echo 'idが正しくありません。';
    exit;
}
# 空欄チェック
...
# ISBN（13桁)チェック
if (!preg_match('/Ad{0,13}z/u', $_POST['isbn'])) {
    echo 'ISBNは数字13桁までです。';
    exit;
}

# 定価（6桁）チェック
if (!preg_match('/Ad{0,6}z/u', $_POST['price'])) {
    echo '定価は数字6桁までです。';
    exit;
}
# 日付必須チェック
...
# 出版日（yyyy-mm-dd）チェック
if (!preg_match('/Ad{4}-d{1,2}-d{1,2}z/u', $_POST['publish'])) {
    echo '日付のフォーマットが違います。';
    exit;
}
# 正しい日付チェック
